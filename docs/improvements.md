...
方針を ADR で決定済みの保留項目（TLS・ログ）は「実装保留・方針は確定」で
あって未決ではない点に注意:

- **§2.3**: TLS 実装本体（組み込み rustls）— 方針は [ADR-0003](adr/0003-tls-via-reverse-proxy.md)
  で「リバースプロキシ終端を正式・組み込みは保留」と決定済み。加えて設定画面内の
  警告強化（visual ベースライン再生成環境が必要、improvement-plan P1-4※2）
- **§4**: PostgreSQL 経路テスト・REST 結合テストの拡充（いずれも実需・実装待ち）
...
improvement-plan-2026-07.md 側の未消化は P4-5（PostgreSQL）・P4-8（実務
サンプル、要スコープ判定）・P4-9（プリセット構成）・P1-4残（設定画面警告）の
4点のみ（詳細と規模はそちら）。
