...
> 「これから何をやるか」はそちらが一次情報。本書は調査記録と対応履歴として
> 引き続き更新する。
...
合算に縮退する注意点も明記。**設定画面内の警告強化（既存 note の warning
化）は未了** — 設定ページは visual regression の fullPage スナップショット
対象のため、ベースライン再生成ができる環境（CI と同一の Playwright
Chromium + フォント）での実施が必要。TLS 自体の実装（rustls オプトイン）は
引き続き v2 検討事項。
...
### 6.3 スキーマ→列定義の自動導出 — 棚卸し済み（2026-07-18、未実装と確認・M23 候補登録）
...
7. **PostgreSQL 実装 or 期待値の明記**（§6.1）— **期待値の明記は対応済み
   ...
   実装自体は improvement-plan P4-5 として実需ドリブンで再評価
8. ~~**SettingsProvider 移行**（§6.2）~~ — **対応済み（M12、PR #13）**。
   ~~**スキーマ→列導出**（§6.3）~~ — **棚卸し済み（2026-07-18、未実装と
   確認し roadmap M23 候補として登録）**
