...
DOM 順: ハンバーガー(≤900px のみ) → ページ名表示 → スペーサー →
検索ピル → ユーザーメニュー。

ページ名表示は**見出し要素にしない**（`<p>` 等）。文書の `h1` は各ページの
`PageHeader`（§6.1）が持つ。シェル側も `h1` にすると 1 ページに h1 が
2 つ並ぶため（実装時に E2E の strict mode 違反として検出済み）。
