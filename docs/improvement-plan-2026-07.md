...
### P4-2: 仮想スクロールの計測ベンチ（実施済み 2026-07-19）

計画当初は「items ページ + 行数パラメータでブラウザ FPS を計測」する案
だったが、ブラウザ FPS は**非決定的・CI で回せない・手動**で、テンプレート
にベンチ用デモルートを焼き込む負債もある。代わりに**仮想化のホットパスを
vitest bench で計測**する方針にした（既存 vitest のみ・依存追加なし・
再現可能・回帰の目視が容易）。

- `packages/grid-svelte/tests/virtual.bench.ts`: (1) per-frame 処理
  （`computeWindow` + 可視ウィンドウ slice）が総行数に依存しないこと
  （仮想化の本質＝1フレームの処理量が O(visible) であること）を 10k/100k で
  実証、(2) sort/filter の総行数依存コスト（sort/filter 変更時に一度走る）を
  計測。`pnpm bench`（root）/ `pnpm --filter @banto/grid-svelte bench`。
- CI には**組み込まない**（timing ベンチは環境依存でマージをゲートすべき
  でない）。`vitest run`（test スクリプト）は `.bench.ts` を拾わないため
  既存テストにも影響なし。
- 代表結果（machine-dependent）と読み方はベンチ冒頭コメントに常設し、
  再実行で更新する運用。spec §4.2 の「1万行・20列 60fps」目標に対し、
  per-frame ≈0.0002ms/frame（16.7ms 予算の4〜5桁下）で余裕を確認。
...
- P4-9（プリセット構成）: **設計完了**（[docs/scaffold-presets-plan.md](scaffold-presets-plan.md)）。
  ...
  実装は P2-1 v2（対話式スキャフォールドの CLI 土台）に乗せるのが自然で、
  その土台が未着手のため設計を先行確定。設計書 §7 の未決事項を実装着手時に
  決めてから進める。
...
- ~~**アプリ内文言の日本語ハードコード**（テンプレートアプリ層のみ）~~:
  **2026-07-22 に方針転換（段階導入）** — template-scope §4.3 の i18n 非導入決定を覆すが、
  今やるのは**パッケージ `@banto/*` の可視文言の外部化（レイヤ①）のみ**で、UI 全体の多言語化
  （②仕組み・③辞書）は保留（[adoption-plan-2026-07.md](adoption-plan-2026-07.md) AD-6 /
  [i18n-plan.md](i18n-plan.md)）。外国人労働者増を実例トリガとする。既定表示は当面日本語のまま。
...
## 続き — 採用者視点レビュー（2026-07-22）

上記2レビューが**内部品質**（コード品質・AI保守性）を対象にしていたのに対し、
**外向きの角度＝導入体験と採用魅力**（AI/作者/他ユーザーから使う体験、他ユーザーが
採用する魅力）で評価したレビューを別途実施した。評価記録は
[adoption-review-2026-07.md](adoption-review-2026-07.md)、そこから導いた実施計画は
[adoption-plan-2026-07.md](adoption-plan-2026-07.md)（AD-1〜AD-6）に分離して記録する。
実質的な新規項目は AD-1〜AD-3（ポジショニング宣言・視覚訴求・リリースタグ運用の起動）で、
本計画の技術項目（P3-2 Tauri CI・P2-2 英語 README・P4-9 scaffold 等）や
[template-scope §7](template-scope.md)（コピー面積削減）とは**採用レンズでの優先度
再評価**として接続する（重複の帰属は adoption-plan §1 の照合表に明示）。

## 続き — ChatGPT 計画レビューの統合（2026-07-22）

上記の採用レビュー・計画に対し ChatGPT が計画レビューを実施した。その具体指摘（意味的
ドリフト7件）を**実コードで全件検証**（全 CONFIRMED）した上で統合した記録は
[plan-review-integration-2026-07.md](plan-review-integration-2026-07.md)。派生した機械検査は
CR-6（rule 8 ロール床照合）/ CR-7（バージョン整合）として
[maintainability-review-2026-07.md](maintainability-review-2026-07.md) に、採用導線の更新
（AD-3 訂正・AD-5 昇格 + 受け入れCI/コールドスタート評価）は adoption-plan に接続した。
最重要は `audit_config_get` の**両経路ロール非対称**（conventions §1 違反の実バグ）で、
P0（① より先・3PR分割）の PR-A で是正する。
