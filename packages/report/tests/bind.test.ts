...
function bindTemplate(template: string, data: unknown, formatters?: Record<string, (v: unknown) => string>) {
...
describe('bind - path resolution', () => {
	...
	it('resolves an undefined path to empty text and records a deduped warning', () => {
		...
		expect(result.blocks).toEqual([
			{ type: 'paragraph', children: [{ type: 'text', value: '' }, { type: 'text', value: ' and ' }, { type: 'text', value: '' }, { type: 'text', value: ' again' }] }
		]);
...
describe('bind - each (block level)', () => {
	...
	it('supports nested each (departments -> staff)', () => {
		const template = '{{#each departments}}\n## {{ name }}\n{{#each staff}}\n- {{ . }}\n{{/each}}\n{{/each}}';
		const data = {
			departments: [
...
describe('bind - if', () => {
	it('includes the block when the path is truthy', () => {
		...
		expect(result.blocks).toEqual([{ type: 'paragraph', children: [{ type: 'text', value: 'See notes.' }] }]);
	});
	...
	it('treats a non-empty array as truthy', () => {
		const result = bindTemplate('{{#if lowStock}}\nRestock needed.\n{{/if}}', { lowStock: [{ name: 'x' }] });
		expect(result.blocks).toEqual([{ type: 'paragraph', children: [{ type: 'text', value: 'Restock needed.' }] }]);
	});
...
describe('bind - formatters', () => {
	...
	it('registers and applies a custom formatter', () => {
		const result = bindTemplate('{{ score | percent }}', { score: 0.5 }, { percent: (v) => `${Number(v) * 100}%` });
		expect(result.blocks).toEqual([{ type: 'paragraph', children: [{ type: 'text', value: '50%' }] }]);
	});

	it('a custom formatter can override a default one', () => {
		const result = bindTemplate('{{ amount | yen }}', { amount: 100 }, { yen: (v) => `JPY${v}` });
		expect(result.blocks).toEqual([{ type: 'paragraph', children: [{ type: 'text', value: 'JPY100' }] }]);
	});

	it('falls back to the raw value and warns on an unknown formatter', () => {
		const result = bindTemplate('{{ amount | bogus }}', { amount: 42 });
		expect(result.blocks).toEqual([{ type: 'paragraph', children: [{ type: 'text', value: '42' }] }]);
